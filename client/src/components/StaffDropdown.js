...
const StaffDropdown = () => {
  ...
  // Fetch staff names from the backend
  useEffect(() => {
    const fetchStaffNames = async () => {
      ...
      try {
        ...
        setError("Error fetching staff names.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-[#DDE5ED] pt-24 min-h-screen">
      <div className="w-full max-w-3xl mx-auto p-8 bg-[#FFFDF5] shadow-lg rounded-lg border border-[#F9A826]">
        <h1 className="text-2xl font-bold text-center text-[#3C4A3E] mb-6">Staff Timetable Viewer</h1>
        ...
        {fileDetails && !loading && (
          <div className="mt-6">
            <h3 className="text-xl font-semibold text-[#3C4A3E] mb-4">File Details for {selectedStaff}</h3>
