...
const LabRoomUpdate = () => {
  ...
  const addLabRoom = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/labrooms", { roomName });
      ...
      <div className="flex justify-around mt-2">
        <button
          ...
          onClick={async () => {
            ...
            try {
              await axios.delete(`http://localhost:5000/api/labrooms/${roomToDelete}`);
      ...
      <div className="flex justify-around mt-2">
        <button
          ...
          onClick={async () => {
            toast.dismiss(t.id); // Close the toast immediately
            try {
              await axios.put(`http://localhost:5000/api/labrooms/${roomToSave}`, {
